// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"encoding/json"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/validate"
)

// CdnCreateSiteRequestFeature The features available to the new CDN site
//
// Multiple products can served on a single CDN site. Features control how those products are managed on the StackPath backend.
//
//  - CDN: The site will have CDN caching abilities
//  - WAF: The site is protected by the StackPath Web Application Firewall
//
// swagger:model cdnCreateSiteRequestFeature
type CdnCreateSiteRequestFeature string

const (

	// CdnCreateSiteRequestFeatureCDN captures enum value "CDN"
	CdnCreateSiteRequestFeatureCDN CdnCreateSiteRequestFeature = "CDN"

	// CdnCreateSiteRequestFeatureWAF captures enum value "WAF"
	CdnCreateSiteRequestFeatureWAF CdnCreateSiteRequestFeature = "WAF"
)

// for schema
var cdnCreateSiteRequestFeatureEnum []interface{}

func init() {
	var res []CdnCreateSiteRequestFeature
	if err := json.Unmarshal([]byte(`["CDN","WAF"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		cdnCreateSiteRequestFeatureEnum = append(cdnCreateSiteRequestFeatureEnum, v)
	}
}

func (m CdnCreateSiteRequestFeature) validateCdnCreateSiteRequestFeatureEnum(path, location string, value CdnCreateSiteRequestFeature) error {
	if err := validate.EnumCase(path, location, value, cdnCreateSiteRequestFeatureEnum, true); err != nil {
		return err
	}
	return nil
}

// Validate validates this cdn create site request feature
func (m CdnCreateSiteRequestFeature) Validate(formats strfmt.Registry) error {
	var res []error

	// value enum
	if err := m.validateCdnCreateSiteRequestFeatureEnum("", "body", m); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// ContextValidate validates this cdn create site request feature based on context it is used
func (m CdnCreateSiteRequestFeature) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}
