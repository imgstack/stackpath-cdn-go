// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// CustconfOriginPullLogs Enable/Disable Origin Pull Logs
//
// swagger:model custconfOriginPullLogs
type CustconfOriginPullLogs struct {

	// enabled
	Enabled bool `json:"enabled"`

	// This is used by the API to perform conflict checking
	ID string `json:"id,omitempty"`
}

// Validate validates this custconf origin pull logs
func (m *CustconfOriginPullLogs) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this custconf origin pull logs based on context it is used
func (m *CustconfOriginPullLogs) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *CustconfOriginPullLogs) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *CustconfOriginPullLogs) UnmarshalBinary(b []byte) error {
	var res CustconfOriginPullLogs
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
